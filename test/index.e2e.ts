...
test('homepage', async ({ page }) => {
  ...
  expect(facebookHref).toEqual(
    'https://www.facebook.com/sharer/sharer.php?u=https%3A%2F%2Fcheetsheet.hhf.technology%2F'
  )
  ...
  expect(twitterHref).toEqual(
    'https://twitter.com/intent/tweet?text=Ridiculous%20collection%20of%20web%20development%20cheatsheets%20https%3A%2F%2Fcheetsheet.hhf.technology%2F'
  )
})
...
test('/knex', async ({ page }) => {
  ...
  const facebookHref = await shareOnFacebook.getAttribute('href')
  expect(facebookHref).toEqual(
    'https://www.facebook.com/sharer/sharer.php?u=https%3A%2F%2Fcheetsheet.hhf.technology%2Fknex'
  )

  const shareOnTwitter = page.getByLabel('Share on Twitter')
  const twitterHref = await shareOnTwitter.getAttribute('href')
  expect(twitterHref).toEqual(
    'https://twitter.com/intent/tweet?text=The%20ultimate%20cheatsheet%20for%20Knex%20https%3A%2F%2Fcheetsheet.hhf.technology%2Fknex'
  )
})
...
test('/sinon', async ({ page }) => {
  ...
  expect(payload2.itemListElement[0].item['@id']).toEqual(
    'https://cheetsheet.hhf.technology/#javascript-libraries'
  )
})
